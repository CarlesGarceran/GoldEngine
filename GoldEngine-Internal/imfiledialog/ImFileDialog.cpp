...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			window->DrawList->AddRectFilled(pos, { (pos.x + size.x), (pos.y + size.y) }, ImGui::ColorConvertFloat4ToU32(ImGui::GetStyle().Colors[(*state & 0b10) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg]));
	...
	void* FileDialog::m_getIcon(const std::filesystem::path& path)
	{
		...
		if (!SHGetFileInfoW(path.c_str(), attrs, &fileInfo, sizeof(fileInfo), flags))
			return { 0 };
		...
		int byteSize = width * height * 4; // RGBA
		std::vector<uint8_t> pixels(byteSize);

		// Extract raw pixel data
		GetBitmapBits(iconInfo.hbmColor, byteSize, pixels.data());
		...
		memcpy(img.data, pixels.data(), byteSize);

		DeleteObject(iconInfo.hbmColor);
		DeleteObject(iconInfo.hbmMask);
		DestroyIcon(fileInfo.hIcon);

		m_icons[pathU8] = this->CreateTexture(&img, img.width, img.height, 0);
	...
	void FileDialog::m_clearIconPreview()
	{
		...
		for (auto& data : m_content) {
			...
			if (data.IconPreviewData != nullptr) {
				RAYLIB::UnloadImage(*data.IconPreviewData);
				delete data.IconPreviewData;
		...
		for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
			...
			if (data.Path.has_extension()) {
				...
				if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" || ext == ".tga") {
					...
					*image = RAYLIB::LoadImage(data.Path.u8string().c_str());
					...
					data.IconPreviewWidth = image->width;
					data.IconPreviewHeight = image->height;
				}
			}
		}
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				if (entry.HasIconPreview && entry.IconPreviewData != nullptr) {
					...
					RAYLIB::UnloadImage(*entry.IconPreviewData);
