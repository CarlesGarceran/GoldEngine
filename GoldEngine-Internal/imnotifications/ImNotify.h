...
public:
     ...
     */
    inline void setType(const ImGuiToastType_& type)
    {
...
public:
     ...
     *
     * @return ImGuiToastType_The type of the toast notification.
     */
    inline ImGuiToastType_ getType()
    {
        return this->type;
    };
     ...
     *
     * @return ImGuiToastPhase_ The current phase of the toast notification.
     ...
     */
    inline ImGuiToastPhase_ getPhase()
    {
        std::chrono::nanoseconds elapsedTime = getElapsedTime();
        const int64_t elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(elapsedTime).count();
    ...
    {
        const ImGuiToastPhase_ phase = getPhase();
...
public:
     ...
     */
    ImGuiToast(ImGuiToastType_ type, int dismissTime = NOTIFY_DEFAULT_DISMISS)
    {
        IM_ASSERT(type < ImGuiToastType::COUNT);
     ...
     */
    ImGuiToast(ImGuiToastType_ type, const char* format, ...) : ImGuiToast(type)
    {
        NOTIFY_FORMAT(this->setContent, format);
    }
     ...
     * @param ... The variable arguments to be formatted according to the format string.
     */
    ImGuiToast(ImGuiToastType_ type, int dismissTime, const char* format, ...) : ImGuiToast(type, dismissTime)
    {
        NOTIFY_FORMAT(this->setContent, format);
    }
     ...
     * @param ... The variable arguments to be formatted according to the format string.
     */
    ImGuiToast(ImGuiToastType_ type, int dismissTime, const char* buttonLabel, const std::function<void()>& onButtonPress, const char* format, ...) : ImGuiToast(type, dismissTime)
    {
        NOTIFY_FORMAT(this->setContent, format);
...
namespace ImGui
{
     ...
     */
    inline void RenderNotifications(ImVec2 pos = GetMainViewport()->Pos, ImVec2 size = GetMainViewport()->Size)
    {
        const ImVec2 mainWindowSize = size;
